...
function* importManifestStructure({
  payload: { manifestUrl, manifest: manifestObj },
}) {
  if (manifestObj.structures) {
    yield put(importStructure(manifestToStructure(manifestObj)));
  }
}

function* saga() {
  yield takeEvery(manifest.MANIFEST_SUCCESS, importManifestStructure);
}
